...
const controller = {
  ...
  generateCards() {
    view.displayCards(utitily.getRandomNumberArray(52));
  },
  // 依照遊戲狀態來做不同行為
...
const utitily = {
  getRandomNumberArray(count) {
    ...
    for (let index = number.length - 1; index > 0; index--) {
      ...
      // console.log(number[index], number[randomIndex],index)
    }
    return number;
  },
};
...
//  監聽器放此處是因為未執行 "view.displayCards()"時,不會有card產生...
